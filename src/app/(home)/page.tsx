...
const tasks = [
  { name: "이메일 로그인", completed: false, percentage: 80 },
  { name: "이메일 회원가입", completed: false, percentage: 80 },
  { name: "소셜로그인", completed: false, percentage: 80 },
  { name: "소셜 회원가입", completed: false, percentage: 80 },
...
export default function HomePage() {
  return (
    <div className="bg-red-100 p-4 md:px-16 lg:px-24 xl:px-60 min-h-[calc(100vh-90px)]">
      <h1 className="text-lg md:text-xl lg:text-2xl font-bold mb-6">작업 진행 상황</h1>
      <p className="text-sm md:text-base mb-2">현재 작업페이지 : 이메일 회원가입</p>
          ...
          <tbody>
            {tasks.map((task, index) => (
              <tr key={index} className="text-center">
                <td className="border px-2 md:px-4 py-2">{task.name}</td>
                ...
                <td className="border px-2 md:px-4 py-2">{task.percentage}%</td>
              </tr>
            ))}
          </tbody>
        </table>
